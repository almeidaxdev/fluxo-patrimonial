...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const resultado = await prisma.$transaction(async (tx) => {
      ...
      if (!podeTransitar(solicitacao.status, 'REJEITADA_PATRIMONIO')) {
      ...
      const atualizadas = await tx.solicitacao.updateMany({
        ...
        data: {
          ...
          motivoRejeicaoPatrimonio: parsed.data.motivo,
        },
      })
      ...
      const payload = construirPayloadRejeicao(dadosParaSnapshot, 'patrimonio')
    ...
    // Fora da transação: sem `aindaValido` de verdade (REJEITADA_PATRIMONIO
    // é status terminal — ver validade-evento.ts), mas o helper é chamado
    // do mesmo jeito por consistência com as demais rotas (retorna
    // `undefined` sozinho, sem custo de leitura extra).
    await processarEmailEvento(
      ...
      { aindaValido: criarValidadorDeEvento('REJEICAO_PATRIMONIO', id) ?? undefined }
    )
