// src/lib/email/templates/solicitacao-aguardando-gestor.ts
// Etapa email-gestor-pendente — template do evento SOLICITACAO_AGUARDANDO_GESTOR.
...
//
// Único destinatário: o gestor indicado na atividade externa (nunca o
// solicitante, nunca o Patrimônio, nunca administradores — ver POST
// /api/solicitacoes, onde este evento é criado). Exclusivo do fluxo
// EXTERNO — reserva interna nunca passa por AGUARDANDO_GESTOR.
//
// Semântica temporal: esta é uma notificação de que HÁ uma decisão
// pendente, não uma garantia de que ela CONTINUA pendente no instante em
// que o gestor lê o e-mail (aindaValido, ver validade-evento.ts, cobre a
// obsolescência antes do envio — mas nada impede o gestor de decidir por
// outro canal entre o envio e a leitura). O CTA leva para dentro do
// sistema, onde o estado real e as ações de aprovar/rejeitar sempre
// refletem a situação atual — nenhum token de aprovação viaja no e-mail.
...
export interface SolicitacaoAguardandoGestorTemplateInput {
  numero: number
  nomeGestor: string
...
function montarLinhasResumo(input: SolicitacaoAguardandoGestorTemplateInput): SummaryRowInput[] {
...
export function renderSolicitacaoAguardandoGestorEmail(input: SolicitacaoAguardandoGestorTemplateInput): RenderedEmail {
  // Mesma convenção de assunto dos demais templates ([Fluxo Patrimonial] +
  // frase + número) — ver reserva-confirmada.ts/pronta-retirada.ts.
  const subject = `[Fluxo Patrimonial] Reserva externa aguardando sua aprovação — #${input.numero}`
  const linhasResumo = montarLinhasResumo(input)
  const avisoAprovacao = 'Esta solicitação depende da sua aprovação para seguir para análise do Patrimônio.'

  const bodyHtml = `
<p style="margin: 0 0 4px 0; font-size: 15px;">Olá, <strong>${escapeHtml(input.nomeGestor)}</strong>.</p>
<p style="margin: 0 0 4px 0;">${escapeHtml(input.nomeSolicitante)} criou uma solicitação de atividade externa que aguarda sua aprovação.</p>
<p style="margin: 0 0 16px 0; font-size: 13px; color: #667085;">${escapeHtml(avisoAprovacao)}</p>
...
`.trim()
  ...
  const blocosTexto = [
    `Olá, ${input.nomeGestor}.`,
    '',
    `${input.nomeSolicitante} criou uma solicitação de atividade externa que aguarda sua aprovação.`,
    '',
    avisoAprovacao,
    '',
